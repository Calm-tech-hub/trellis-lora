...
class Trainer:
    ...
    def run(self):
        ...
        Run training.
        """
        skip_dataset_snapshot = os.environ.get('TRELLIS_SKIP_DATASET_SNAPSHOT', '0') == '1'
        skip_init_snapshot = os.environ.get('TRELLIS_SKIP_INIT_SNAPSHOT', '0') == '1'

        if self.is_master:
            print('\nStarting training...')
            if not skip_dataset_snapshot:
                self.snapshot_dataset()
        if self.step == 0:
            if not skip_init_snapshot:
        ...
        while self.step < self.max_steps:
            ...
            if self.is_master:
                ...
                if self.step % self.i_log == 0:
                    ## save to log file
                    log_str = '\n'.join([
                        f'{step}: {json.dumps(log, default=lambda x: x.item() if isinstance(x, np.generic) else x)}' for step, log in log
                    ])
