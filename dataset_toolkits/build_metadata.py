...
if __name__ == '__main__':
        ...
        with ThreadPoolExecutor(max_workers=os.cpu_count()) as executor, \
            ...
            def worker(sha256):
                try:
                    ...
                    rendered = bool(metadata.loc[sha256, 'rendered'])
                    if need_process('rendered') and rendered == False and \
                        os.path.exists(os.path.join(opt.output_dir, 'renders', sha256, 'transforms.json')):
                        updates['rendered'] = True
                        rendered = True

                    voxelized = bool(metadata.loc[sha256, 'voxelized'])
                    if need_process('voxelized') and rendered == True and voxelized == False and \
                        ...
                        try:
                            ...
                            updates['voxelized'] = True
                            updates['num_voxels'] = len(pts)
                            voxelized = True
                    ...
                    for model in ss_latent_models:
                        if need_process(f'ss_latent_{model}') and \
                            ...
                            updates[f'ss_latent_{model}'] = True

                    if len(updates) > 0:
                        with update_lock:
                            for key, value in updates.items():
                                metadata.loc[sha256, key] = value
                    pbar.update()
                except Exception as e:
    ...
    with open(os.path.join(opt.output_dir, 'statistics.txt'), 'w') as f:
        ...
        if 'captions' in metadata.columns:
            caption_count = metadata['captions'].count()
        elif 'caption' in metadata.columns:
            caption_count = metadata['caption'].count()
        f.write(f'  - Number of assets with captions: {caption_count}\n')
