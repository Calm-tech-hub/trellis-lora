...
def main(local_rank, cfg):
    ...
    if hasattr(cfg, 'pretrained') and cfg.pretrained is not None:
        for name, path in cfg.pretrained.items():
            if name not in model_dict:
                raise ValueError(f'Pretrained target {name} not found in model_dict')
            pretrained_model = models.from_pretrained(path)
            state_dict = pretrained_model.state_dict()
            model_dict[name].load_state_dict(state_dict, strict=True)
            del pretrained_model
            if rank == 0:
                print(f'[Pretrained] Loaded {name} from {path}')

    if hasattr(cfg, 'lora') and cfg.lora is not None and cfg.lora.get('enabled', False):
        lora_targets = cfg.lora.get('targets', ['self_attn.to_qkv', 'self_attn.to_out', 'cross_attn.to_q', 'cross_attn.to_kv', 'cross_attn.to_out'])
        lora_rank = cfg.lora.get('rank', 8)
        lora_alpha = cfg.lora.get('alpha', 8.0)
        lora_dropout = cfg.lora.get('dropout', 0.0)
        lora_models = cfg.lora.get('models', list(model_dict.keys()))
        for name, model in model_dict.items():
            trainable = name in lora_models
            for param in model.parameters():
                param.requires_grad = False if trainable else param.requires_grad
            if not trainable:
                continue
            ...
            if len(replaced) == 0:
                raise ValueError(f'LoRA enabled for model {name}, but no target linear layers were matched.')
            if rank == 0:
                print(f'[LoRA] Applied to model {name}: {len(replaced)} layers')
                for layer_name in replaced:
                    print(f'  - {layer_name}')
